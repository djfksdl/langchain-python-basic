...
# OpenAI 채팅 모델 생성
chat = ChatOpenAI( # 객체 = 클래스 / 여기는 옛날 방식임
    model="gpt-3.5-turbo", #or gpt-4 oepn-ai 모델 선택부분은 공식 홈페이지에서 확인 가능합니다. 현재는 gpt-3.5-turbo 모델을 사용하겠습니다.
    temperature=0, openai_api_key=OPENAI_API_KEY # 온도는 시드값이라해서 0~1값인데 0에 가까울수록 같은 말 뱉을확률이 높고, 1에 가까우면 변동됨
)

# 질문 정의
# question = "기분이 우울한 날에는 어떤것을 하면 기분이 나아질수 있을까?"
question = "친구와 싸웠는데 어떻게 화해하면 좋을까?"
...
# 질문에 대한 응답 생성
try:
    response = chat.invoke(messages) # invoke는 호출하다.  
    print(response)
